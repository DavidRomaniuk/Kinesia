...
using Kinesia.Gestion.Migrations.Seed.Host;
...
namespace Kinesia.Gestion.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : GestionAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<GestionDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{GestionConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{GestionConsts.ConnectionStringName}", constring);
        }
