...
using Kinesia.Gestion.Schemas;
using Kinesia.Gestion.Web.Chat.SignalR;
using Kinesia.Gestion.Web.Common;
using Kinesia.Gestion.Web.Resources;
using Swashbuckle.AspNetCore.Swagger;
using Kinesia.Gestion.Web.IdentityServer;
using Kinesia.Gestion.Web.Swagger;
using Stripe;
...
using Kinesia.Gestion.Web.HealthCheck;
using Owl.reCAPTCHA;
using Microsoft.AspNetCore.Server.Kestrel.Https;
using Microsoft.Extensions.DependencyInjection.Extensions;
using Kinesia.Gestion.Web.Extensions;
using Kinesia.Gestion.Web.MultiTenancy;
using SecurityStampValidatorCallback = Kinesia.Gestion.Identity.SecurityStampValidatorCallback;

namespace Kinesia.Gestion.Web.Startup
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            return services.AddAbp<GestionWebMvcModule>(options =>
            {
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, ILoggerFactory loggerFactory)
        {
            ...
            if (env.IsDevelopment())
            {
                ...
                app.UseGestionForwardedHeaders();
            }
            else
            {
            ...
            if (GestionConsts.PreventNotExistingTenantSubdomains)
            {
            ...
            if (WebConsts.SwaggerUiEnabled)
            {
                ...
                {
                    options.SwaggerEndpoint(_appConfiguration["App:SwaggerEndPoint"], "Gestion API V1");
                    options.IndexStream = () => Assembly.GetExecutingAssembly()
                        .GetManifestResourceStream("Kinesia.Gestion.Web.wwwroot.swagger.ui.index.html");
        ...
        private void ConfigureSwagger(IServiceCollection services)
        {
            services.AddSwaggerGen(options =>
            {
                ...
                var applicationXml = $"Kinesia.Gestion.Application.xml";
                var applicationXmlPath = Path.Combine(AppContext.BaseDirectory, applicationXml);
                options.IncludeXmlComments(applicationXmlPath);

                var webCoreXmlFile = $"Kinesia.Gestion.Web.Core.xml";
